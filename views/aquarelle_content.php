<?php
$title = "L'aquarelle";
$class = "alt style2";
//define the variables for use with template
$im1 = './assets/images/pic11.jpg';
$im1a = 'Ballon rouge et collinne verte';
$bname = "Halte-accueil l'Aquarelle";
$bcontent = 'Ballon Rouge ASBL';
$s1name = 'Historique';
$s1content = "« L'Aquarelle », avec  « Le Ballon Rouge », service d’accueillantes conventionnées, constitue l’ASBL «  Le Ballon Rouge » née en 1992 à l’initiative de Vie Féminine Bruxelles.";
$s21im = './assets/images/pic12.jpg';
$s21ima = "une aquarelle d'une petite fille tenant un ballon rouge";
$s2h1name = 'Vocations';
$s2h1content = "Implantée au cœur de Laeken, cette halte-accueil est née à l’initiative des femmes qui se sont organisées entre elles pour garder les enfants durant les activités et les cours organisés par Vie Féminine à la Maison Mosaïque.

Progressivement, l’accueil s’est professionnalisé et le service a été autorisé par l’ONE.  Un tel service démontre chaque jour sa nécessité pour permettre aux personnes d’accéder à l’emploi via des formations qualifiantes. Les parents ne peuvent suivre une formation et s’y concentrer que si leurs enfants sont pris en charge pendant ce temps, et s’ils sont rassurés sur la qualité du service.";
$s22im = './assets/images/pic13.jpg';
$s22ima = "Une aquarelle de mains";
$s2h2name = "Objectifs";
$s2h2content = "L’Aquarelle a pour mission d’accueillir 18 ETP (Equivalent Temps Plein ) enfants de parents ayant un projet individuel et/ou professionnel à mettre en œuvre (formation ou un programme de réinsertion professionnelle, recherche d’emploi ou effectuer un travail occasionnel, prendre un temps de repos, effectuer des démarches administratives…).";
$s3name = "Status";
$s3content = "L'aquarelle est subventionnée par ACTIRIS, le Fonds social européen (FSE) et le Fonds d'équipements et de services collectifs (FESC)";
?>
